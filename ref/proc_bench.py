...
for line in sys.stdin:
    ...
    if ":" in line:
        ...
        parsed = parse.parse(
            "{keygen_time:f} ({keygen_cyc:d} cycles)  {sign_time:f} ({sign_cyc:d} cycles)  {verify_time:f} ({verify_cyc:d} cycles)",
            line.strip(),
        )
...
print(
    f"{name} & {keygen_time[len(keygen_time)>>1]:f} & {keygen_cyc[len(keygen_cyc)>>1]}"
