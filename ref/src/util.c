...
void XOF(uint8_t **buf, size_t *length, const uint8_t *seed, size_t seed_len,
  ...
  for (int i = 0; i < num; i++) shake256_squeeze(buf[i], length[i], &shake);
}
...
GFq_t rnd_GF(keccak_state *shake) {
  ...
  while (val >= MEDS_p) {
    ...
    val = 0;
    for (int i = 0; i < sizeof(GFq_t); i++) val |= data[i] << (i * 8);
      ...
      for (int r = 0; r < M_r; r++) {
        ...
        for (int r2 = r + 1; r2 < M_r; r2++) {
          ...
          for (int c = r; c < M_c; c++) {
            ...
            pmod_mat_set_entry(tmp, M_r, M_c, r, c,
        ...
        for (int c = r; c < M_c; c++) {
          uint64_t tmp0 =
              ((uint64_t)pmod_mat_entry(tmp, M_r, M_c, r, c) * val) % MEDS_p;
...
int parse_hash(uint8_t *digest, int digest_len, uint8_t *h, int len_h) {
  ...
  while (i < MEDS_w) {
    ...
    for (int j = 0; j < MEDS_t_bytes; j++) pos |= buf[j] << (j * 8);

    pos = pos & MEDS_t_mask;

    if (pos >= MEDS_t) continue;
...
int solve(pmod_mat_t *A, pmod_mat_t *B_inv, pmod_mat_t *G0prime, GFq_t Amm) {
  ...
  for (int i = 0; i < MEDS_m; i++)
    for (int j = 0; j < MEDS_n; j++)
      M[j * (MEDS_m + MEDS_m + 2) + i] =
          (MEDS_p - P0prime1[i * MEDS_n + j]) % MEDS_p;
  ...
  for (int j = 0; j < MEDS_n; j++)
    M[j * (MEDS_m + MEDS_m + 2) + MEDS_m + MEDS_n] =
        ((uint32_t)P0prime0[(MEDS_m - 1) * MEDS_n + j] *
         (MEDS_p - (uint32_t)Amm)) %
        MEDS_p;

  for (int j = 0; j < MEDS_n; j++)
    M[j * (MEDS_m + MEDS_m + 2) + MEDS_m + MEDS_n + 1] =
        ((uint32_t)P0prime1[(MEDS_m - 1) * MEDS_n + j] * (uint32_t)Amm) %
        MEDS_p;

  // LOG_MAT(M, MEDS_n, MEDS_m + MEDS_m + 2);

  if (pmod_mat_syst_ct(M, MEDS_n - 1, MEDS_m + MEDS_m + 2) < 0) return -1;
  ...
  for (int i = 0; i < MEDS_n; i++)
    sol[MEDS_n * MEDS_n - MEDS_n + i] =
        ((uint32_t)P0prime0[(MEDS_m - 1) * MEDS_n + i] * (uint32_t)Amm) %
        MEDS_p;
  ...
  for (int i = 0; i < MEDS_n; i++)
    for (int j = 0; j < MEDS_n - 1; j++) {
      uint32_t tmp =
          (uint32_t)sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 2 * MEDS_n + i] +
          MEDS_p -
          ((uint32_t)M[i * (MEDS_m + MEDS_m + 2) + MEDS_n + MEDS_n - 2 - j] *
           (uint32_t)sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 2 - j]) %
              MEDS_p;
  ...
  for (int i = 0; i < MEDS_n; i++)
    for (int j = 0; j < MEDS_n - 1; j++) {
      uint32_t tmp =
          (uint32_t)sol[MEDS_n * MEDS_n - MEDS_n + i] + MEDS_p -
          ((uint32_t)N[i * (MEDS_n) + MEDS_m - 2 - j] *
           (uint32_t)sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 2 - j]) %
              MEDS_p;
    ...
    for (int i = 0; i < MEDS_n; i++)
      for (int j = 0; j < MEDS_n; j++) {
        uint32_t tmp =
            ...
            MEDS_p -
            ((uint32_t)M[i * (MEDS_m + MEDS_m + 2) + MEDS_n + MEDS_n - 1 - j] *
             (uint32_t)sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 1 -
                           (block - 2) * MEDS_n - j]) %
                MEDS_p;
        sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - block * MEDS_n + i] =
            tmp % MEDS_p;
    ...
    for (int i = 0; i < MEDS_n; i++)
      for (int j = 0; j < MEDS_n; j++) {
        uint32_t tmp = sol[MEDS_n * MEDS_n - block * MEDS_n + i] + MEDS_p -
                       ((uint32_t)N[i * (MEDS_n) + MEDS_m - 1 - j] *
                        (uint32_t)sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 1 -
                                      (block - 1) * MEDS_n - j]) %
                           MEDS_p;
...
pmod_vec_mask_t solve_vec(pmod_vec_t *A, pmod_vec_t *B_inv, pmod_vec_t *G0prime,
                          pmod_vec_t Amm) {
  const pmod_vec_t p = SET1(MEDS_p);
  const pmod_vec_t zero = SET1(0);
  const pmod_vec_t one = SET1(1);

  pmod_vec_t P0prime0[MEDS_m * MEDS_n];
  pmod_vec_t P0prime1[MEDS_m * MEDS_n];
  ...
  pmod_vec_t N[MEDS_n * MEDS_m];

  for (int i = 0; i < MEDS_m; i++)
    for (int j = 0; j < MEDS_n; j++)
      N[j * MEDS_m + i] = GF_mod_vec(SUB(p, P0prime0[i * MEDS_n + j]));

  pmod_vec_t M[MEDS_n * (MEDS_m + MEDS_m + 2)] = {0};

  for (int i = 0; i < MEDS_m; i++)
    for (int j = 0; j < MEDS_n; j++)
      M[j * (MEDS_m + MEDS_m + 2) + i] =
          GF_mod_vec(SUB(p, P0prime1[i * MEDS_n + j]));
  ...
  for (int j = 0; j < MEDS_n; j++)
    M[j * (MEDS_m + MEDS_m + 2) + MEDS_m + MEDS_n] =
        GF_mod_vec(MULLO(P0prime0[(MEDS_m - 1) * MEDS_n + j], SUB(p, Amm)));

  for (int j = 0; j < MEDS_n; j++)
    M[j * (MEDS_m + MEDS_m + 2) + MEDS_m + MEDS_n + 1] =
        GF_mod_vec(MULLO(P0prime1[(MEDS_m - 1) * MEDS_n + j], Amm));

  pmod_vec_mask_t valid =
      pmod_mat_syst_ct_vec(M, MEDS_n - 1, MEDS_m + MEDS_m + 2);
  ...
  for (int r = 0; r < MEDS_n - 1; r++) {
    ...
    for (int c = MEDS_n - 1; c < MEDS_m + MEDS_m + 1; c++) {
      pmod_vec_t tmp0 =
          pmod_mat_entry(M, MEDS_n, MEDS_m + MEDS_m + 2, MEDS_n - 1, c);
      pmod_vec_t tmp1 = pmod_mat_entry(M, MEDS_n, MEDS_m + MEDS_m + 2, r, c);

      pmod_vec_t val = GF_mod_vec(MULLO(tmp1, factor));
  ...
  for (int r = 0; r < MEDS_n - 1; r++) {
    ...
    for (int c = MEDS_n; c < MEDS_m + MEDS_m + 1; c++) {
      ...
      pmod_vec_t val = GF_mod_vec(MULLO(tmp0, factor));
  ...
  pmod_vec_t sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m] = {0};
  ...
  for (int i = 0; i < MEDS_n; i++)
    sol[MEDS_n * MEDS_n - MEDS_n + i] =
        GF_mod_vec(MULLO(P0prime0[(MEDS_m - 1) * MEDS_n + i], Amm));
  ...
  for (int i = 0; i < MEDS_n; i++)
    for (int j = 0; j < MEDS_n - 1; j++) {
      ...
      sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 2 * MEDS_n + i] = GF_mod_vec(tmp);
    }
  ...
  for (int i = 0; i < MEDS_n; i++)
    for (int j = 0; j < MEDS_n - 1; j++) {
      pmod_vec_t tmp = SUB(
          ...
          GF_mod_vec(MULLO(N[i * (MEDS_n) + MEDS_m - 2 - j],
                           sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 2 - j])));
      sol[MEDS_n * MEDS_n - MEDS_n + i] = GF_mod_vec(tmp);
    }
      ...
      for (int j = 0; j < MEDS_n; j++) {
        pmod_vec_t tmp =
            SUB(ADD(sol[MEDS_n * MEDS_n - block * MEDS_n + i], p),
                GF_mod_vec(MULLO(N[i * (MEDS_n) + MEDS_m - 1 - j],
                                 sol[MEDS_n * MEDS_n + MEDS_m * MEDS_m - 1 -
                                     (block - 1) * MEDS_n - j])));
        sol[MEDS_n * MEDS_n - block * MEDS_n + i] = GF_mod_vec(tmp);
      }
...
void G_mat_init(pmod_mat_t *G, pmod_mat_t *Gsub[MEDS_k]) {
  for (int i = 0; i < MEDS_k; i++) Gsub[i] = G + i * MEDS_m * MEDS_n;
}
...
void pi(pmod_mat_t *Gout, pmod_mat_t *A, pmod_mat_t *B, pmod_mat_t *G) {
  ...
  for (int i = 0; i < MEDS_k; i++) {
    pmod_mat_mul(Gsub[i], MEDS_m, MEDS_n, A, MEDS_m, MEDS_m, G0sub[i], MEDS_m,
                 MEDS_n);
    pmod_mat_mul(Gsub[i], MEDS_m, MEDS_n, Gsub[i], MEDS_m, MEDS_n, B, MEDS_n,
                 MEDS_n);
  }
}
...
void pi_vec(pmod_vec_t *Go, pmod_vec_t *A, pmod_vec_t *B, pmod_vec_t *G0) {
  ...
  for (int i = 0; i < MEDS_k; i++) {
    pmod_mat_mul_vec(Go_sub[i], MEDS_m, MEDS_n, A, MEDS_m, MEDS_m, G0_sub[i],
                     MEDS_m, MEDS_n);
    pmod_mat_mul_vec(Go_sub[i], MEDS_m, MEDS_n, Go_sub[i], MEDS_m, MEDS_n, B,
                     MEDS_n, MEDS_n);
  }
}
