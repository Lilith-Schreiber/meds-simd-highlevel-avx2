...
void pmod_mat_mul_x16(pmod_mat_x16_t *C, int C_r, int C_c, pmod_mat_x16_t *A, int A_r,
  ...
  for (int c = 0; c < C_c; c++)
    for (int r = 0; r < C_r; r++) {
      ...
      for (int i = 0; i < A_r; i++) {
        val = ADD_x16(val, MULLO_x16(pmod_mat_entry(A, A_r, A_c, r, i),
...
pmod_mat_mask_x16_t pmod_mat_syst_ct_x16(pmod_mat_x16_t *M, int M_r, int M_c) {
  ...
  pmod_mat_mask_x16_t valid = pmod_mat_inv_x16(M_inv, M, M_r, M_c);
  pmod_mat_mask_mul_x16(M, M_r, M_c, M_inv, M_r, M_r, M, M_r, M_c, M_r);

  return valid;
}

pmod_mat_mask_x16_t pmod_mat_inv_x16(pmod_mat_x16_t *M_inv, pmod_mat_x16_t *M, int M_r, int M_c) {
  const pmod_mat_x16_t zero = SET1_x16(0);
  ...
  for (int r = 0; r < M_r; r++) {
    for (int r2 = r + 1; r2 < M_r; r2++) {
      ...
      __mmask16 Mrr_eq_zero = EQ_x16(Mrr, zero);

      for (int c = r; c < M_r; c++) {
        pmod_mat_x16_t val = pmod_mat_entry(M_prime, M_r, M_r, r2, c);
        pmod_mat_x16_t Mrc = pmod_mat_entry(M_prime, M_r, M_r, r, c);
        pmod_mat_x16_t res = ADD_M_x16(Mrc, val, Mrr_eq_zero);
        res = SUB_M_x16(res, p, GE_x16(res, p));
    ...
    __mmask16 val_neq_zero = NEQ_x16(val, zero);
    valid = valid & val_neq_zero;

    val = GF_inv_x16(val);
    for (int c = r; c < M_r; c++) {
      pmod_mat_x16_t Mrc = pmod_mat_entry(M_prime, M_r, M_r, r, c);
      pmod_mat_x16_t res = GF_reduc_x16(MULLO_x16(Mrc, val));
      pmod_mat_set_entry(M_prime, M_r, M_r, r, c, res);
    }
    for (int c = 0; c < M_r; c++) {
      pmod_mat_x16_t Mrc = pmod_mat_entry(M_inv, M_r, M_r, r, c);
    ...
    for (int r2 = r + 1; r2 < M_r; r2++) {
      ...
      for (int c = r; c < M_r; c++) {
        ...
        pmod_mat_x16_t val = GF_reduc_x16(MULLO_x16(tmp0, factor));
      ...
      for (int c = 0; c < M_r; c++) {
        ...
        val = GF_reduc_x16(MULLO_x16(tmp0, factor));
        val = SUB_x16(tmp1, val);
        val = ADD_M_x16(val, p, LT_x16(val, zero));
